...
async function seedMovies(client) {
  try {
    ...
    const insertedMovies = await Promise.all(
      movies.map(
        (movie) => client.sql`
        INSERT INTO movies (poster, title, year, imdb_rating)
          VALUES (${movie.poster}, ${movie.title}, ${movie.year}, ${movie.imdb.rating})
          ON CONFLICT DO NOTHING;
      `,
      ),
    );

    console.log(`Seeded ${insertedMovies.length} movie`);
